"""The command line entry point for ReflectionPT."""
import datetime
import functools
import logging
import os
...
logger = logging.getLogger("ReflectionPT")
...
@click.option(
    ...
    required=True,
    default="denovo",
    help="\b\nThe mode in which to run ReflectionPT:\n"
...
def main(
    ...
    if output is None:
        output = os.path.join(
            os.getcwd(),
            f"ReflectionPT_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}",
        )
    else:
    ...
    # Run ReflectionPT in the specified mode.
    if mode == "denovo":
        logger.info("Predict peptide sequences with ReflectionPT.")
        writer = None
        # writer.set_metadata(
        ...
        logger.info("Evaluate a trained ReflectionPT model.")
        model_runner.evaluate(peak_path, model, config)
    elif mode == "train":
        logger.info("Train the ReflectionPT model.")
