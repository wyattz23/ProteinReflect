...
def predict(
    ...
    """
    Predict peptide sequences with a trained ReflectionPT model.
...
def evaluate(peak_path: str, model_filename: str, config: Dict[str,
                                                               Any]) -> None:
    """
    Evaluate peptide sequence predictions from a trained ReflectionPT model.
...
def _execute_existing(
    ...
    """
    Predict peptide sequences with a trained ReflectionPT model with/without
    evaluation.
    ...
    if not config["temp_dir_auto"]: # if we don't use auto-generated temp dir
        ...
        tmp_dir = MyDirectory("/mnt/petrelfs/zhangxx/dump6/")
    else:
